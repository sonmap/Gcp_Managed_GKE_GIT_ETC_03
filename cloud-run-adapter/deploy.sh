...
: "${TARGET_PROJECT_ID:?Set TARGET_PROJECT_ID to the project where the PoC BigQuery dataset will be created}"
...
For production, require authentication and have the Java portal obtain an OIDC identity token or place the service behind an approved internal access path.
EOF
